...
Badge.Text = "COUNTER BLOX"
Badge.Size = UDim2.new(0, 96, 0, 18)
...
credit.Position = UDim2.new(0, 220, 0, 0)
...
local function CreateSpectrumPicker(parent, default, callback)
    ...
    sv.InputBegan:Connect(function(i) if (i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch) then dragSV = true end end)
    hue.InputBegan:Connect(function(i) if (i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch) then dragHue = true end end)
...
CreateDropdown(combatPage, "Target", {"Head", "HeadHB", "HumanoidRootPart"}, Config.AimbotTarget, 5, function(v) Config.AimbotTarget = v end)
...
if UserInputService.TouchEnabled and not UserInputService.MouseEnabled and not UserInputService.KeyboardEnabled then
    ...
    mAim.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then AimbotActive = true end
    end)
    mAim.InputEnded:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.Touch or i.UserInputType == Enum.UserInputType.MouseButton1 then AimbotActive = false; CurrentTarget = nil end
    end)
end
